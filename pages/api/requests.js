import redis, { errorResponseError, invalidMethodError, noCaseBody, okResponseBody, verifyUserAsAdmin, getStringDate } from '@/lib/redis'
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (await verifyUserAsAdmin(userEmail)) {
        ...
        // If the request is to get a specific request object (that can be anything in case of admin)
          ...
          res.status(requestObj ? 200 : 400).json(requestObj ? { code: 1, requestObj } : errorResponseError)
          return
        }
    ...
    // Add Upcoming Requests
      ...
      // Check if it's just a slot request
        ...
        const timeSlotObj = JSON.parse(await redis.hget(`${spaceObj['name']}_timeslots`, `${getStringDate(requestObj['dateBookedFor'])}_${requestObj['startTime']}_${requestObj['endTime']}`))
