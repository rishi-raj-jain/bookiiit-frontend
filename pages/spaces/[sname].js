...
const Space = ({ sname, userData }) => {
  ...
  useEffect(() => {
    fetch(`/api/spaces?spaceName=${sname}`)
      .then((res) => res.json())
      .then((res) => {
  ...
  return !userData || Object.keys(spaceData).length < 1 ? null : (
    <div className="mx-auto mt-12 flex max-w-7xl flex-col px-4">
      <div className="flex flex-col space-y-5 md:flex-row md:space-y-0 md:space-x-2">
        {spaceData && spaceData.name && (
          <div className="md:w-1/2">
            <img src={spaceData.image} alt={spaceData.name} className="h-auto w-full rounded object-cover object-center shadow-sm" />
          </div>
        )}
        <div className="p-5 md:w-1/2">
          {spaceData && spaceData.name && <h1 className="text-3xl font-bold">{spaceData.name}</h1>}
          {spaceData && spaceData.capacity && <p className="mt-3 text-xl">Capacity: {spaceData.capacity}</p>}
          {spaceData && spaceData.spaceType && (
            <p className="text-md mt-5">
              <b>Type:</b> {spaceData.spaceType}
            </p>
          )}
          <button className={`rounded border p-5 focus:outline-none ${spaceData.ifDirect ? 'text-teal-600' : 'text-red-700'} mt-3 mr-2 mb-2 py-2.5 text-sm font-medium`}>
            {spaceData.ifDirect ? 'Can be directly booked' : 'Can not be directly booked'}
          </button>
        </div>
      </div>
      <div className="mt-10">
        <h1 className="text-3xl font-bold dark:text-white">Book Your Space</h1>
        <form
          onSubmit={(e) => {
            e.preventDefault()
            let requestObj = {
        ...
        >
            ...
            <>
              ...
              <div className="mt-5 flex flex-row items-center">
                ...
                <button
                  ...
                  className="ml-2 inline-flex w-12 items-center justify-center whitespace-nowrap rounded border border-transparent bg-teal-600 py-[5px] px-4 text-base text-white shadow-sm hover:bg-teal-700 dark:border-white"
                >
                  Add
                </button>
              </div>
          ...
          <button
            type="submit"
            className="mt-3 inline-flex w-36 items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-teal-600 px-4 py-2 text-base text-white shadow-sm hover:bg-teal-700 dark:border-white"
          >
            Book &rarr;
          </button>
        </form>
        <div className="mt-10"></div>
        <TimeSlots setStartTime={setStartTime} setEndTime={setEndTime} spaceName={sname} selectedDate={requestDate} globalTime={globalTime} />
      </div>
    </div>
  )
}
